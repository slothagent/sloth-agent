import { Connection, PublicKey, ConfirmedSignatureInfo, ParsedTransactionWithMeta, ParsedInstruction, TransactionSignature } from '@solana/web3.js';
  ...
  private async getTokenMetadataWithRetry(mint: string, maxRetries = 5): Promise<any> {
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        const metadata = await Metadata.fromAccountAddress(this.connection, metadataAddress);
        ...
        if (error.toString().includes('429') || error.toString().toLowerCase().includes('too many requests')) {
          ...
          // Switch endpoints before retrying
          await this.handleConnectionError(error);
          continue;
        }
...
// Helper function to fetch token metadata with retry logic
const getTokenMetadata = async (connection: Connection, mint: string, maxRetries = 5): Promise<any> => {
  let retryCount = 0;
  let lastError: any;
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (metadata.data.uri) {
        try {
          // Add delay before fetching off-chain metadata
          await sleep(500);
          const response = await fetch(metadata.data.uri);
          if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
