...
const TableTransaction = ({tx, ethPrice, sonicPrice}: TableTransactionProps) => {
    ...
    return (
        <div key={tx.transactionHash} className="grid grid-cols-12 gap-4 p-4 text-sm hover:bg-[#1C2333] transition-colors duration-200">
            ...
            <a target='_blank' href={`${tx.network == "Sonic" ? "https://testnet.sonicscan.org/address/" : "https://scanv2-testnet.ancient8.gg/address/"}${tx.to}`} className="col-span-3 text-gray-400 hover:text-white hover:underline">
                {formatAddress(tx.to)}
            </a>
            <a target='_blank' href={`${tx.network == "Sonic" ? "https://testnet.sonicscan.org/token/" : "https://scanv2-testnet.ancient8.gg/token/"}${tx.from}`} className="col-span-3 text-gray-400 hover:text-white hover:underline">
