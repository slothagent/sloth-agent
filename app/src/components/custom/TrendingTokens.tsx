    ...
    <div>
      ...
      <Card className="border-2 border-black bg-white rounded-none shadow-[4px_4px_0px_0px_rgba(0,0,0)]">
        <Table>
          ...
          <TableBody>
            {tokens.map((row) => (
              <TableRow key={row.id} className="border-b border-black text-black hover:bg-[#93E905]/10 cursor-pointer" onClick={() => router.push(`/agent/${row.token.symbol.toLowerCase()}`)}>
